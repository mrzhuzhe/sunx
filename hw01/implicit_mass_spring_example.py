...
new_dv = ti.Vector.field(2,dtype=ti.f32, shape=max_num_particles)
...
def substep():
    ...
    for i in range(n):
        ...
        #total_force = ti.Vector.field(gravity, float) * particle_mass
...
def substep_jacobi():
    ...
    for i in range(n) :
        #b[i] = ti.Vector.field(gravity, float) * dt
        b[i] = gravity * dt
        for j in range(n) :
